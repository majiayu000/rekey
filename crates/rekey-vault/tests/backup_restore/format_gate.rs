...
async fn restore_rejects_format_twelve_without_installing_or_migrating() {
    ...
    db.execute_batch("PRAGMA writable_schema=ON; UPDATE sqlite_schema SET sql=replace(sql,'format_version = 13','format_version = 12') WHERE name='vault_header'; PRAGMA writable_schema=OFF;").unwrap();
    drop(db);
    let db = rusqlite::Connection::open(&backup).unwrap();
