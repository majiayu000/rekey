...
- 存活 Broker 的绝对 deadline 和错误执行 kill/wait 直属 bwrap；future 取消使用 kill-on-drop。fork 前拒绝 setuid/setgid/filecaps 的 `/usr/bin/bwrap`。`pre_exec` 在降低 rlimit 之前双 fork 一个已 `setsid` 的监护进程，并用握手字节确认它在 `exec` 之后进入循环；监护进程不调用 `unshare`。Broker 仍存活时，监护进程记下 launcher 的子进程。Broker 死亡而 launcher 还活着时，先枚举再 `SIGKILL` 整棵树和 launcher 自己的进程组。取消和错误路径在 Broker 进程里先枚举 `/proc/<pid>/task/*/children`，从叶子开始 `SIGKILL`，然后再杀 launcher。这样不依赖 launcher 死后进程组是否还在。Broker 自己被 SIGKILL 时 Drop 不会运行，仍由监护进程处理。不再对插件 bwrap 使用 `--die-with-parent` 或外层 `PR_SET_PDEATHSIG`：那只会先杀掉外层进程，留出 bubblewrap 内部 `raw_clone` 到 `do_init` 之间的孤儿窗口。这不是内核级原子父子同死，直接 `SIGKILL` launcher 时，最后一次扫描之后新出现、又来不及被记下的子进程仍可能留下。pre_exec 仍不分配、不加锁。
- artifact 快照（打开、读取、摘要、拷贝、复核）在 `spawn_blocking` 中执行，并与插件交换共用同一个绝对 `timeout_at(deadline)`。超时或拷贝返回后已过期则拒绝 spawn/exec。阻塞拷贝不可中断：超时只停止等待，后台线程结束时仍 drop 私有目录。不引入第二套看门狗线程。Broker 进程被 SIGKILL 时仍可能留下 `rekey-issue-plugin-*` 目录，本切片不扫描/tmp 回收他人快照。不引入 cgroup/`memory.max`：无额外 capability 的 LinuxKit 验收不能写物理总量限额。

验收必须运行最终生产 allowlist，而不是用研究 denylist 替代：同一真实 artifact 两操作、恶意输出零 exchange、文件/UDS/网络/进程及 namespace 拒绝、只读 root、FD500 后降低 limit 和过滤器 FD 无泄漏、自 exec 后限额仍在、CPU/AS、超量/挂起/deadline、快照拷贝超过 deadline 时不得 spawn、取消、READY 后父死，以及 LAUNCHER 已出现但尚未 READY 时的第一跳父死。所有关键攻击有未隔离成功控制。LinuxKit arm64 容器证据不代表原生 Ubuntu 或 x86_64 实证。任何系统依赖不满足都失败，不将沙箱启动错误算攻击防御成功。人工安全审查仍为合并门槛。
