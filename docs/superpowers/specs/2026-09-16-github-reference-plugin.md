...
- 存活 Broker 的绝对 deadline 和错误执行 kill/wait 直属 bwrap；future 取消使用 kill-on-drop。fork 前拒绝 setuid/setgid/filecaps 的 `/usr/bin/bwrap`。`pre_exec` 在降低 rlimit 之前双 fork 一个已 `setsid` 的监护进程，并用握手字节确认它在 `exec` 之后进入循环；监护进程不调用 `unshare`。Broker 仍存活时，监护进程记下 launcher 的子进程。Broker 死亡而 launcher 还活着时，先枚举再 `SIGKILL` 整棵树和 launcher 自己的进程组。launcher 被 `kill_on_drop` 直接杀死时，也立刻向该进程组发 `SIGKILL`：组长这时仍是 zombie，pgid 还没有复用；seccomp 拒绝 payload 自己的 `setsid`。不再对插件 bwrap 使用 `--die-with-parent` 或外层 `PR_SET_PDEATHSIG`：那只会先杀掉外层进程，留出 bubblewrap 内部 `raw_clone` 到 `do_init` 之间的孤儿窗口。这不是内核级原子父子同死，直接 `SIGKILL` launcher 时，最后一次扫描之后新出现、又来不及被记下的子进程仍可能留下。pre_exec 仍不分配、不加锁。
