...
- 存活 Broker 的绝对 deadline 和错误执行 kill/wait bwrap；future 取消使用 kill-on-drop，由 Tokio 回收直属子进程，通过 PID namespace 终止 payload。取消测试与 READY 后父死测试要求已启动子树停止运行，允许暂存待收割 zombie。fork 前拒绝 setuid/setgid/filecaps 的 `/usr/bin/bwrap`。`pre_exec` 在现有 rlimit/FD 处理后调用 `PR_SET_PDEATHSIG(SIGKILL)` 并核对 `getppid` 仍为捕获的 Broker PID，以关闭第一跳注册竞态；不分配、不加锁。这不关闭 bubblewrap 内部 `raw_clone` 之后、`do_init` 重设 PDEATHSIG 之前的窗口，也不能称为启动全阶段原子父死。
