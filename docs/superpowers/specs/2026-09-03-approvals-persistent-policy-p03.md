...
operator can pin it independently of the envelope. `rekey approval pending` lists
unused in-memory challenges; `rekey approval get UUID` reprints a freshly
origin-signed envelope. Both are Admin reads of live session state, not a GUI,
push inbox, or Agent-callable sign socket. `execute --approval` accepts one or two repeated paths to regular
