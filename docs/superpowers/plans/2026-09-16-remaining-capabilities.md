...
| APR-09 | 通知与审批操作界面 | 本机详情审阅实施中；通知渠道及托管部分待输入 |
| APR-10 | 人员目录与组织关系 | 依赖 ENT-03 的稳定人员身份 |
| AUD-06 | 审计保留与删除 | 可先规格化显式 Admin cutoff 清理；必须保留完整 execution 链并使旧分页快照显式失效。自动保留策略另需时限及责任约定 |
...
| DYN-05 | 租约续期 | 待规格：固定 provider 操作、任务期限与取消关系 |
| DYN-06 | 持久租约及重启清理 | 待规格：租约持久化、秘密归属、崩溃与解锁恢复 |
...
| EXT-06 | OS Keychain 凭证源 | 待规格：与已实现解锁包装区分，明确访问主体 |
| EXT-07 | 通用签名/Provider | 待输入：第一个固定操作，避免任意秘密代理 |
| KEY-04 | VRK/DEK 轮换 | 下一本地切片：同 VRK 下全版本 DEK 原子轮换；完整 VRK 轮换还需重封派生状态、重新钉扎审批来源及恢复因素合同 |
| NET-07 | Agent 可见流式响应 | 待规格：未完整读取前如何维持反射密封，不能直接透传 |
| OS-05 | macOS 隔离启动器 | 待规格与独立攻击验收；不能套用 Linux 结论 |
...
| VEX-04 | KV 最新版与写入 | 待规格：精确授权、版本漂移和写入不确定性 |
| P-08 | 可观测性 | 本机快照实施中；不把该切片算作 OTel/远程采集完成 |
| P-10 | Connector 隔离 | 待规格：独立执行边界、效果准入与资源限额 |
...
- DYN-05 可限定到当前固定 Action 的绝对期限内续期，保留同一租约与最终 revoke。它是后续工程切片，不需要新 provider；实施前须核实 Vault renewal API。
...
- 外部部分由独立规格 lane 产出 `2026-09-16-external-capabilities.md`，不创建空实现。
