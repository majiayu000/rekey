...
| Linux `agent-run` netns launcher (`linux-netns-v1`) | Implemented | — | `rekey agent-run` / `rekeyd agent-run` via system bubblewrap; disjoint `--agent-socket` required; after `--tmpfs /tmp` the canonical socket is bind-mounted back so a `/tmp` endpoint remains reachable; macOS `UNSUPPORTED_PLATFORM`; contract tests in `rekey-domain`/`rekey-broker` sandbox modules; black-box `scripts/p9-linux-agent-run.sh` on Linux CI. Ubuntu exact-head was blocked by an ERR-trap false failure on expected overlap exit 2 plus the `/tmp` overlay; those launcher/script fixes are in this revision and still require a green Ubuntu re-run before Black-box Verified. Not general G2, not a Docker-harness replacement, not in `v2.0.0-alpha.1`. |
...
| Vault KV v2 fixed-version CredentialSource | Black-box Verified | — | Development head only: typed encrypted source profile, exact version/key read, public HTTPS screen, shared absolute deadline, no retry, source/resolved-value sealing, fixed-Action injection, typed rotation, drain gate, restart and backup/restore passed with release `rekeyd` + `rekey`, dual UDS, SQLite and local CA/TLS fixtures in `vault_source_contract` and `scripts/p7-vault-kv-source.sh`. No live Vault interoperability or release inclusion. OSS-binary protocol interop is specified in `docs/superpowers/specs/2026-09-07-vault-oss-interop.md` and is not implemented. |
| Vault one-shot dynamic lease CredentialSource | Black-box Verified | — | Development head only: typed encrypted profile, one exact `creds` acquisition, 5–300 second lease bound, selected-value sealing, fixed-Action injection, exact synchronous revoke-before-success, bounded malformed-response cleanup, non-retryable uncertainty, audit-failure cleanup and lock drain pass in `vault_dynamic_contract`. No renewal, durable lease registry, crash-time cleanup guarantee, live Vault interoperability, private-network source access, or release inclusion. OSS-binary protocol interop is specified in `docs/superpowers/specs/2026-09-07-vault-oss-interop.md` and is not implemented. |
