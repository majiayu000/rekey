...
| Linux `agent-run` netns launcher (`linux-netns-v1`) | Implemented | — | `rekey agent-run` / `rekeyd agent-run` via system bubblewrap; disjoint `--agent-socket` required; macOS `UNSUPPORTED_PLATFORM`; contract tests in `rekey-domain`/`rekey-broker` sandbox modules; black-box `scripts/p9-linux-agent-run.sh` on Linux CI. Not general G2, not a Docker-harness replacement, not in `v2.0.0-alpha.1`. |
