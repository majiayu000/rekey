...
| GitHub CreateIssue reference process | Black-box Verified | Unreleased | macOS bundled native sidecar with separate deny-default Seatbelt profile; only canonical public issue input/output, Broker retains secrets/effects/revoke/audit. Eight focused attack cases and real Broker revoke-before-success fixture passed. Sampling RSS is not a hard limit; no general SDK-04 registration or full P-10 claim. See `2026-09-16-github-reference-plugin.md`. |
| Action-bound GitHub issue plugin registration | Black-box Verified | Unreleased | Optional `github_issue_plugin` stores Admin-approved path/SHA-256/protocol on immutable Action versions using existing step-up CRUD. Seven real Broker fixtures prove two different native artifacts actually execute, mismatches/missing/symlink/replacement stop before remote effects, version pin/disable, audit rollback, restart and backup roundtrip. Explicit binding never falls back to bundled code; only macOS runtime verified. Schema12 rejects v11 and older; backups include declarations, not executable files. See `2026-09-16-action-plugin-registration.md`. |
| Independent Anthropic text streaming | Black-box Verified | Unreleased | `execute-text-stream`; eight real local TLS/UDS cases prove checked early text before upstream completion, late failure, audit failure, disconnect, deadline and slow consumer. Four parser/sealer cases include all split points; CLI validates terminal/sequence/EOF; MCP rejects projection. Only completed is success; visible checked prefixes cannot be recalled. Opaque-token, one text block, no tools/thinking or live provider validation. Original Execute remains buffered. Current state/backup format 12 rejects earlier versions. See `2026-09-16-anthropic-text-stream.md`. |
