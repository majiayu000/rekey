...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Init empty v7 vault | `rekey init` / `rekeyd init` | Black-box Verified | `v2.0.0-alpha.1` (v5 artifact) | `crates/rekey-vault/src/bootstrap.rs`, `crates/rekey-broker/src/bin/rekeyd.rs` | `cargo test -p rekey-vault --test bootstrap_contract`; `cargo test -p rekey-cli --test cli_blackbox` | empty password, nonempty dir, legacy/non-v7 state, unknown/NULL crypto discriminator, confirm mismatch discards | Development head is breaking v7 with no v5/v6 reader or migration; Alpha artifact remains v5 | README Quick start |
...
| Connector contract SDK / MCP and OAuth projections | Contract Tested | — | IO-free `rekey-connector` format v1 registry describes only `fixed-http-header@1` and `github-app-installation@1`; Broker routes through it while retaining credential/network/deadline/audit/sealing/revoke ownership. Public testkit covers registry/effect/lifecycle invariants and routing mismatch; pure MCP object-schema projection rejects `x-mcp-header`, and OAuth projection serializes only fixed public RFC 8693 metadata. No MCP server, live generic OAuth, dynamic plugin/registry, new Agent API, provider expansion, release inclusion, or broader G1/G2 claim. |
...
| External CredentialSource | Specified | — | The connector contract exists, but no HashiCorp Vault, cloud KMS/secrets, 1Password, HSM, keychain, remote source, generic sign endpoint, or secret-bearing provider adapter is implemented |
