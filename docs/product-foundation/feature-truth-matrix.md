...
Human desktop current-secret reveal and API-key add use a password-authenticated 15-minute Admin session (source only). Locked/stale/forged sessions and the Agent channel are denied; plaintext is returned only in Admin response bodies after durable audit commits. Verified by `scripts/test-human-vault.py` and `admin_ipc::desktop_values_use_body_and_agent_channel_cannot_reveal`. Native interaction automation for this extension remains pending.
