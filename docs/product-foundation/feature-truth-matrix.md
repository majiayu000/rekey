...
| --- | --- | --- | --- |
| Local Admin metrics snapshot (P-08 slice) | Contract Tested | — | `rekey metrics [--prometheus]` samples fixed numeric process counters and capability gauges through existing Admin IPC, including locked state. Metrics reads do not extend idle lock; Agent access, malformed requests and hostile replies are rejected. Six focused metrics tests passed on 2026-09-16. No listener, sensitive labels, OTel, persistent collection or worker queue-depth claim. See `2026-09-16-local-metrics.md` and the remaining-capabilities plan. |
