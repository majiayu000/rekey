...
with tempfile.TemporaryDirectory(prefix='rk-backup-drill-',dir='/tmp') as tmp:
 ...
 with (work/'broker.log').open('w') as log:
  try:
   ...
   call(ssh+['umask 077; mkdir -p /Users/apple/Documents/rekey-backups && mkdir '+remote_dir])
   call(['scp','-q','-o','BatchMode=yes',str(archive),str(receipt_file),'apple:'+remote_dir+'/'])
