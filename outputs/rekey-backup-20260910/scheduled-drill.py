import shutil,hashlib,importlib.util,json,os,secrets,subprocess,sys,tempfile,time
...
with tempfile.TemporaryDirectory(prefix='rk-backup-drill-',dir='/tmp') as tmp:
 ...
 with (work/'broker.log').open('w') as log:
  try:
   ...
   # Transfer only encrypted material and its non-secret receipt to the authorized host.
   remote_dir='/Users/apple/Documents/rekey-backups/drill-'+time.strftime('%Y%m%dT%H%M%SZ',time.gmtime())+'-'+secrets.token_hex(4)
   receipt_file=work/'receipt.json'
   receipt_file.write_text(json.dumps({k:v for k,v in receipt.items() if k!='output_path'},indent=2)+'\n')
   ssh=['ssh','-o','BatchMode=yes','-o','ConnectTimeout=15','apple']
   queued=Path.home()/'.local/share/rekey-backups/outbox'/Path(remote_dir).name
   queued.mkdir(mode=0o700)
   shutil.copyfile(archive,queued/'snapshot.rkbackup')
   shutil.copyfile(receipt_file,queued/'receipt.json')
   try:
    label='gui/'+str(os.getuid())+'/io.github.majiayu000.rekey.backup-sync'
    call(['launchctl','kickstart',label])
    deadline=time.monotonic()+60
    while True:
     time.sleep(3)
     job=call(['launchctl','print',label]).decode()
     if 'state = not running' in job:
      assert 'last exit code = 0' in job, 'scheduled job failed; inspect backup-sync.stderr.log'
      break
     assert time.monotonic()<deadline, 'scheduled job timed out'
   finally:
    shutil.rmtree(queued)
   remote_hash=call(ssh+['shasum -a 256 '+remote_dir+'/snapshot.rkbackup']).decode().split()[0]
   assert remote_hash==digest
   remote_modes=call(ssh+["stat -f '%Lp' "+remote_dir+' '+remote_dir+'/snapshot.rkbackup '+remote_dir+'/receipt.json']).decode().split()
   assert remote_modes==['700','600','600']
   downloaded=work/'downloaded.rkbackup'
   call(['scp','-q','-o','BatchMode=yes','apple:'+remote_dir+'/snapshot.rkbackup',str(downloaded)])
