            ...
            try:
                ...
                snapshot = {'format_version':3,'version':1,'expires_at_ms':int((expires+600)*1000),'approvers':[],
                    'workload_identities':[{'principal_id':principal,'issuer':claims['iss'],
                        ...
                        'profile':{'kind':'ci-cloud','subject':claims['sub']},'keys':[],'online_key_source':'github-actions-jwks'}],
                ...
                run(mint,(unused+'\n').encode(),expected=4)
                # A fresh token for the same expiry identity must still mint:
                # outage/key failure must not masquerade as the expired-token check.
                fresh = token(audience + '/expiry')
                assert decode(fresh.split('.')[1])['exp'] > time.time()
                fresh_session = json.loads(run(mint, (fresh+'\n').encode()))
                # Force buffered broker logs to disk before scanning evidence.
                run(base+['shutdown','--password-stdin'],proof)
                proc.wait(timeout=15)
                for needle in [jwt,unused,wrong,fresh,password,canary,session['capability_token'],fresh_session['capability_token']]:
                ...
                receipt = {'issuer':claims['iss'],'subject':claims['sub'],'audience':audience,
                    'kid':header['kid'],'online_key_source':'github-actions-jwks',
                    'jwks_url':'https://token.actions.githubusercontent.com/.well-known/jwks','inline_policy_key_count':0,
                    'jwt_saved':False,'mint_success':True,'fixed_read_status':200,'replay_exit':4,'wrong_audience_exit':4,
                    'naturally_expired_unused_jwt_exit':4,'fresh_same_audience_after_expiry_mint_success':True,'expiry_epoch':expires,'tested_at_epoch':int(time.time()),
                    'source_base':'358b3a5','source_fix':'bounded RS256 x5t plus broker fresh fixed GitHub JWKS',
                    'source_files_sha256':{p:hashlib.sha256((Path('source')/p).read_bytes()).hexdigest() for p in ['crates/rekey-policy/src/workload.rs','crates/rekey-policy/src/lib.rs','crates/rekey-broker/src/runtime.rs','crates/rekey-broker/src/runtime/workload.rs']},'runner_commit':os.environ['GITHUB_SHA'],'binary_sha256':{Path(p).name:hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in [rk,rd]},
                    'limitation':'Fixed GitHub issuer/JWKS only; fresh fetch per mint without cache; no general OAuth exchange; anonymous upstream read'}
            finally:
